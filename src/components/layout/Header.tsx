...
export function Header() {
    ...
    return (
        <header className="sticky top-0 z-50 w-full bg-surface-dark border-b border-gray-800 shadow-sm">
            <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    {/* Logo */}
                    <Link href="/" className="flex-shrink-0 flex items-center gap-3 group">
                        <div className="size-10 bg-primary/20 rounded-lg flex items-center justify-center text-primary group-hover:bg-primary/30 transition-colors">
                    ...
                    {/* Mobile Menu Button */}
                    <button
                        className="md:hidden p-2 rounded-lg text-slate-300 hover:bg-gray-800"
...
function MobileNavLink({ href, children, onClick }: { href: string; children: React.ReactNode; onClick: () => void }) {
    return (
        <Link
            href={href}
            className="block px-3 py-3 text-base font-medium text-slate-300 bg-transparent hover:bg-gray-800 hover:text-primary rounded-lg transition-colors"
            onClick={onClick}
        >
            {children}
        </Link>
